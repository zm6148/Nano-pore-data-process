...
for channel in channels:
    ...
    event_index_pair_no_repeat = []
    if len(event_index_pair) == 0:
        print('no event in this channel, continue')
        continue
    else:
        for ii in range(1, len(event_index_pair)):
            if ii == 0:
                event_index_pair_no_repeat.append(event_index_pair[ii])
            else:
               if (event_index_pair[ii][0] != event_index_pair[ii-1][0]) and (event_index_pair[ii][1] != event_index_pair[ii-1][1]):
                   event_index_pair_no_repeat.append(event_index_pair[ii])   
    ...
    ############################################################################
    ## find data not in the segment
    a = segment_pair_selected
    for ele in segment_pair_selected:
        a = np.where((a >= ele[1]) & (a <=ele[0]))
        
    
    ############################################################################
    ## alocate detected events to corrsponding windows
    events_within_partition = []
    new_segment_pair_selected = []
    for window in segment_pair_selected:
        start_bond = window[0]
        end_bond = window[1]
        temp = []
        for ele in event_index_pair_no_repeat:
            if ele[0] >= start_bond and ele[1] <= end_bond:
                temp.append(ele)
        events_within_partition.append(temp)
        #data_from_last_event_to_segment_end = r_current_np[temp[-1][1] : end_bond]
        #data_between_events = r_current_np[temp[-1][1] : end_bond]
        
    if empty(events_within_partition):
        print('no event in this channel, continue')
        continue
        
    ## remove empty entry in events_within_partition
    not_empty_events_within_partition = []
    for ele in events_within_partition:
        if len(ele) > 0:
            not_empty_events_within_partition.append(ele)
        
    ############################################################################
    ## remove long gap between last event and the end of correspoding segment
    ...
    for ii in range(0, len(not_empty_events_within_partition)):
        
        channel_total_time = channel_total_time + (segment_pair_selected[ii,1] - segment_pair_selected[ii,0])/sr
        #channel_total_event = channel_total_event + len(not_empty_events_within_partition[ii]) 
        ...
        ## 'event depth',
        ## 'event interval
        for jj in range(0,len(not_empty_events_within_partition[ii])):
            
            event_start_time = not_empty_events_within_partition[ii][jj][0]
            event_end_time = not_empty_events_within_partition[ii][jj][1]
            event_width = (not_empty_events_within_partition[ii][jj][1] - not_empty_events_within_partition[ii][jj][0])/sr
            ...
            if jj == 0:
                ...
                event_interval = (not_empty_events_within_partition[ii][jj][0] - not_empty_events_within_partition[ii][jj-1][1])/sr
    ...
    if len(channel_event_detial) == 0:
        print('no event with range in this channel, continue')
        continue
